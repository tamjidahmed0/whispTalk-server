...
const mediaUploadEvent = ({ io, socket, ONLINE_USERS_KEY }) => {
    ...
    try {
        ...
        socket.on('mediaUpload', async (data) => {
            ...
            data.text?.map((item) => {
                io.to(online_user).emit('mediaUpload', {
                ...
                io.to(onlineUser_sender).emit("mediaUpload_sender", {
                    messageId: data.messageId,
                    iSend: senderId,
                    text: item,
                    type: type,
...
export default mediaUploadEvent